...
const statusConfig: Record<string, { label: string; color: 'warning' | 'info' | 'primary' | 'success' | 'danger' | 'secondary' }> = {
  ...
  'em_execucao': { label: 'Em Execução', color: 'primary' },
...
export default function OSPage() {
  ...
  // Serviços e produtos do formulário
  ...
  const gerarHTMLImpressao = (os: OrdemServico, baseUrl: string) => {
    // Logo em base64 para funcionar na janela de impressão
      ...
      <body>
        ...
        <div class="totais">
          ...
          <table>
            <tbody>
              <tr>
                <td>TOTAL DE HORAS/QTDE DE SERVIÇOS</td>
        ...
        ${os.observacoes ? `<div class="observacoes"><strong>Observações:</strong> ${os.observacoes}</div>` : ''}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Filtrar serviços e produtos vazios
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-dark-400">Gerencie orçamentos de serviços e produtos</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="card p-4">
          <p className="text-dark-400 text-sm">Orçamentos</p>
          <p className="text-2xl font-bold text-yellow-400">
            {ordensServico.filter(os => os.status === 'orcamento').length}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-dark-400 text-sm">Em Execução</p>
          <p className="text-2xl font-bold text-blue-400">
            {ordensServico.filter(os => os.status === 'em_execucao').length}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-dark-400 text-sm">Concluídos</p>
      ...
      <Modal
        ...
        title={editingOS ? `Editar Orçamento #${editingOS.numero}` : 'Novo Orçamento'}
        size="xl"
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-dark-800 rounded-lg p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-dark-400">Total Serviços:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-dark-300 mb-2">Observações (aparece na impressão)</label>
              <textarea
            ...
            <div>
              <label className="block text-sm text-dark-300 mb-2">Observações Internas</label>
              <textarea
                value={formData.observacoes_internas}
                onChange={(e) => setFormData({ ...formData, observacoes_internas: e.target.value })}
                className="input w-full h-24"
                placeholder="Observações internas (não aparece na impressão)..."
              />
            </div>
          </div>
